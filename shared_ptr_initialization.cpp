...
class Dog {
  public:
    ...
    explicit Dog(int age) : age_(age) {
        std::cout << "Constructor called, age: " << age << '\n';
    }

    explicit Dog(std::string name) : name_(name) {
        std::cout << "Constructor called, name: " << name << '\n';
    }
...
int main() {
    ...
    std::cout << "Integer ptr 1, use_count: " << integer_ptr_1.use_count() << '\n';
    std::cout << '\n';

    // Move and copy initialization
    std::shared_ptr<int> integer_ptr_2 = std::move(integer_ptr_1);
    std::cout << "Integer ptr 1, use_count: " << integer_ptr_1.use_count() << '\n';
    std::cout << "Integer ptr 2, use_count: " << integer_ptr_2.use_count() << '\n';
    std::cout << '\n';

    std::shared_ptr<int> integer_ptr_3 = integer_ptr_2;
    std::cout << "Integer ptr 1, use_count: " << integer_ptr_1.use_count() << '\n';
    std::cout << "Integer ptr 2, use_count: " << integer_ptr_2.use_count() << '\n';
    std::cout << "Integer ptr 3, use_count: " << integer_ptr_3.use_count() << '\n';
    std::cout << '\n';
    ...
    dog_ptr_3.reset();
    std::cout << "dog ptr 3, use_count: " << dog_ptr_3.use_count() << '\n';
    std::cout << '\n';

    std::shared_ptr<Dog> dog_ptr_4;
    dog_ptr_4.reset(new Dog());

    std::cout << "dog ptr 2, use_count: " << dog_ptr_2.use_count() << '\n';
    std::cout << "dog ptr 4, use_count: " << dog_ptr_4.use_count() << '\n';
    std::cout << '\n';
    ...
    std::cout << "Dog name: " << dog_shared_ptr->name() << '\n';
